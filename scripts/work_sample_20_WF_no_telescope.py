...
output_folder = r"plots/sample_20_WF_no_telescope"
...
spectra_path = r"\\AMIPC045962\Cache (D)\daily_data\spectra\20251128 - Sample 20 WF No telescope"
...
reference_key = list(spectra_data_raw.keys())[0]
reference_intensity = spectra_data_raw[reference_key][:, 1]
for key in spectra_data_white.keys():
    intesity = spectra_data_white[key][:, 1]-reference_intensity
    intensity = intensity / reference_intensity
    spectra_data_subtracted[key] = np.column_stack((spectra_data_white[key][:, 0], intensity))
...
# Plot reference
plot_spectra(spectra_data_subtracted, spectra_params_white, cutoff=1000, new_fig=True)
