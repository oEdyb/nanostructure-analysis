...
output_folder = r"plots/sample_20_12.1"
...
spectra_path = r"\\AMIPC045962\Cache (D)\daily_data\spectra\20251201 - Sample 20"
...
# Subtract each raw spectrum from reference
spectra_data_subtracted = {}
reference_key = list(spectra_data_white.keys())[0]
reference_intensity = spectra_data_white[reference_key][:, 1]
for key in spectra_data_raw.keys():
    intensity = reference_intensity - spectra_data_raw[key][:, 1]
    intensity = intensity / reference_intensity
    spectra_data_subtracted[key] = np.column_stack((spectra_data_raw[key][:, 0], intensity))
...
# Plot reference subtracted
plot_spectra(spectra_data_subtracted, spectra_params_raw, cutoff=1000, new_fig=True)
plt.savefig(os.path.join(output_folder, "reference_subtracted.png"))
plt.close()

# Plot normalized spectra
plot_spectra(spectra_data, spectra_params_raw, cutoff=1000, new_fig=True)
plot_spectra(spectra_data_ref_normalized, spectra_params_white, cutoff=1000, new_fig=False, linestyle="--")
plt.savefig(os.path.join(output_folder, "spectra_all.png"))
plt.close()
