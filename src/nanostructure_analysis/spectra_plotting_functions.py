...
from . import apd_functions, config, spectra_functions
...
def _group_color_map(sorted_keys, fallback_colors):
    ...
    for key in sorted_keys:
        ...
        fallback = (
            default_cycle[idx % len(default_cycle)]
            if default_cycle
            else fallback_colors[idx % len(fallback_colors)]
        )
...
def plot_spectra(
        ...
        if group_colors:
            ...
            if match:
                ...
                if group_letter in groups_in_legend:
                    ...
                    if (
                        group_letter in sem_averages
                        and "gap_width" in sem_averages[group_letter]
                    ):
    ...
    plt.grid(
        True,
        alpha=config.DEFAULT_GRID_ALPHA,
        linestyle="--",
        linewidth=config.DEFAULT_GRID_LINEWIDTH,
    )
    if legend_flag:
        ...
        fontsize = (
            ...
            else config.DEFAULT_LEGEND_FONTSIZE
        )
        legend = plt.legend(
            loc="best", fontsize=fontsize, frameon=False, title=legend_title
        )
        if legend_title:
...
def plot_spectra_integrated_histogram(
    spectra_data, spectra_params, cutoff=950, new_fig=True, bins="auto"
):
    ...
    ax = plt.gca()
    ax.set_xlabel(
        "Integrated Intensity (a.u. nm)", fontsize=config.DEFAULT_LABEL_FONTSIZE
    )
    ...
    ax.tick_params(
        axis="both",
        which="major",
        labelsize=config.DEFAULT_TICK_FONTSIZE,
        width=1.5,
        length=6,
    )
...
def plot_spectra_wavelength_histogram(
    spectra_data, spectra_params, wavelength_target=852, new_fig=True, bins="auto"
):
    ...
    ax = plt.gca()
    ax.set_xlabel(
        f"Intensity at {wavelength_target} nm (a.u.)",
        fontsize=config.DEFAULT_LABEL_FONTSIZE,
    )
    ax.set_ylabel("Count", fontsize=config.DEFAULT_LABEL_FONTSIZE)
    ax.set_title(
        f"Spectra Histogram @ {wavelength_target} nm",
        fontsize=config.DEFAULT_TITLE_FONTSIZE,
    )
    ax.grid(
        True,
        alpha=config.DEFAULT_GRID_ALPHA,
...
def plot_derivative_wavelength_histogram(
    spectra_data, spectra_params, wavelength_target=852, new_fig=True, bins="auto"
):
    ...
    for key in sorted_keys:
        ...
        sampled_derivative = np.interp(
            wavelength_target, wavelength_midpoints, derivative
        )
    ...
    ax = plt.gca()
    ax.set_xlabel(
        f"dI/dλ at {wavelength_target} nm (a.u./nm)",
        fontsize=config.DEFAULT_LABEL_FONTSIZE,
    )
    ax.set_ylabel("Count", fontsize=config.DEFAULT_LABEL_FONTSIZE)
    ax.set_title(
        f"Derivative Histogram @ {wavelength_target} nm",
        fontsize=config.DEFAULT_TITLE_FONTSIZE,
    )
    ax.grid(
        True,
...
def plot_spectra_std_scatter(
    ...
    for data in spectra_data.values():
        wavelengths = data[:, 0]
        intensities = data[:, 1]
        if len(wavelengths) != len(reference_wavelengths) or not np.allclose(
    ...
    if normalize_by_mean:
        ...
        plt.title(
            "Spectra Relative Spread vs Wavelength",
            fontsize=config.DEFAULT_TITLE_FONTSIZE,
        )
    else:
        plt.ylabel("Standard Deviation (a.u.)", fontsize=config.DEFAULT_LABEL_FONTSIZE)
        plt.title(
            "Spectra Standard Deviation vs Wavelength",
            fontsize=config.DEFAULT_TITLE_FONTSIZE,
        )
    plt.grid(
        True,
...
def plot_spectra_savgol(
    ...
    plt.plot(
        wavelength[mask],
        intensity[mask],
        label=f"Raw - {first_key} - {time_str}",
        alpha=0.7,
    ...
    for window in window_sizes:
        for order in orders:
            ...
            plt.plot(
                wavelength[mask],
                filtered_y[mask],
                label=f"Savgol w={window}, o={order}",
                linewidth=config.DEFAULT_LINE_WIDTH * 0.4,
                alpha=0.9,
            )
...
def plot_spectra_transparent(
    spectra_data, spectra_params, cutoff=950, new_fig=True, linestyle="-"
):
    ...
    colors = (
        plt.cm.tab10(range(10)) if len(sorted_keys) <= 10 else plt.cm.tab20(range(20))
    )
...
def plot_spectra_derivative(
    ...
    color_by_group = (
        _group_color_map(sorted_keys, fallback_colors) if group_colors else {}
    )
...
def plot_integrated_transmission_scatter(spectra_data, cutoff=950, new_fig=True):
    ...
    plt.ylabel(
        "Integrated transmission (a.u.* nm)", fontsize=config.DEFAULT_LABEL_FONTSIZE
    )
    plt.title(
        "Integrated Transmission by Group", fontsize=config.DEFAULT_TITLE_FONTSIZE
    )
    plt.grid(
        True,
        linestyle="--",
...
if __name__ == "__main__":
    ...
    spectra_data_box1, spectra_params_box1 = spectra_functions.filter_spectra(
        spectra_data, spectra_params, "*box1*"
    )
    spectra_data_5um, spectra_params_5um = spectra_functions.filter_spectra(
        spectra_data, spectra_params, "*5um*"
    )
    spectra_data_bkg, spectra_params_bkg = spectra_functions.filter_spectra(
        spectra_data, spectra_params, "*bkg*"
    )
